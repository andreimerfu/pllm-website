    ...
    <div className="relative w-full max-w-6xl mx-auto bg-gradient-to-br from-slate-50 to-slate-100 rounded-3xl p-8 lg:p-12 border border-slate-200">
      ...
      <div className="relative z-10">
        ...
        <div className="text-center mb-12">
          <h3 className="text-2xl lg:text-3xl font-bold text-slate-900 mb-4">System Architecture</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12 items-center">
          ...
          <div className="space-y-6">
            <div className="text-center mb-8">
              <h4 className="text-lg font-bold text-slate-900 mb-2">Client Layer</h4>
              <p className="text-sm text-slate-600">Applications & Services</p>
            </div>
            
            <div className="space-y-4">
              {[
                { name: "Web Applications", icon: "🌐", desc: "React, Vue, Angular" },
                { name: "Mobile Apps", icon: "📱", desc: "iOS, Android, React Native" },
                { name: "Backend Services", icon: "⚙️", desc: "Node.js, Python, Go" },
                { name: "AI Platforms", icon: "🤖", desc: "LangChain, AutoGPT" }
              ].map((client, index) => (
                <div key={index} className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200 hover:shadow-md transition-all duration-200">
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-500 truncate">{client.desc}</p>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="relative">
            <div className="text-center mb-8">
              <h4 className="text-lg font-bold text-slate-900 mb-2">pLLM Gateway</h4>
              <p className="text-sm text-slate-600">Intelligent Routing Engine</p>
            </div>
            ...
            <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-3xl p-8 text-white shadow-xl">
              ...
              <div className="text-center mb-8">
                ...
                <p className="text-purple-100 text-sm">High-performance Go runtime</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                {[
                  ...
                  <div key={index} className="bg-white/10 rounded-xl p-3 backdrop-blur-sm">
                    <p className="font-semibold text-sm mb-1">{component.name}</p>
              ...
              <div className="bg-white/10 rounded-xl p-4 backdrop-blur-sm">
                <div className="flex items-center justify-center space-x-2 mb-3">
                  ...
                  <span className="font-semibold">Intelligent Load Balancer</span>
                </div>
          ...
          <div className="space-y-6">
            <div className="text-center mb-8">
              <h4 className="text-lg font-bold text-slate-900 mb-2">Provider Layer</h4>
              <p className="text-sm text-slate-600">LLM Service Providers</p>
            </div>
            
            <div className="space-y-4">
              {[
                { name: "OpenAI", icon: "simple-icons:openai", color: "from-green-500 to-green-600", status: "healthy" },
                { name: "Anthropic", icon: "simple-icons:anthropic", color: "from-orange-500 to-orange-600", status: "healthy" },
                { name: "Azure OpenAI", icon: "logos:microsoft-azure", color: "from-blue-500 to-blue-600", status: "degraded" },
                { name: "AWS Bedrock", icon: "logos:aws", color: "from-yellow-500 to-orange-500", status: "healthy" },
                { name: "Google Vertex", icon: "logos:google-cloud", color: "from-blue-400 to-blue-500", status: "healthy" },
                { name: "Groq", icon: "simple-icons:meta", color: "from-purple-500 to-purple-600", status: "failed" }
              ].map((provider, index) => (
                <div key={index} className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200 hover:shadow-md transition-all duration-200">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <div className="w-10 h-10 bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl flex items-center justify-center border border-slate-200">
                        <iconify-icon icon={provider.icon} class="w-6 h-6"></iconify-icon>
                      </div>
                      <div className="flex-1 min-w-0">
                    ...
                    <div className="text-right">
                      <p className="text-xs text-slate-500">
                        {provider.status === 'healthy' ? '99.9%' : 
                         provider.status === 'degraded' ? '85.2%' : '0%'}
                      </p>
        ...
        <div className="mt-12 pt-8 border-t border-slate-200">
          <div className="text-center mb-6">
            <h5 className="text-lg font-bold text-slate-900 mb-2">Data Flow & Features</h5>
            <p className="text-sm text-slate-600">Real-time monitoring and intelligent routing</p>
          </div>
          
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { name: "Circuit Breaker", icon: "⚡", desc: "Automatic failover protection" },
              { name: "Health Checks", icon: "💓", desc: "Continuous monitoring" },
              { name: "Rate Limiting", icon: "🚦", desc: "Traffic control & quotas" },
        ...
        <div className="mt-8 bg-gradient-to-r from-blue-50 to-purple-50 rounded-2xl p-6">
          <div className="text-center mb-6">
            <h5 className="text-lg font-bold text-slate-900 mb-2">Live Performance Metrics</h5>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { metric: "1000+", label: "Requests/sec", color: "text-blue-600" },
              ...
              <div key={index} className="text-center">
                <p className={`text-2xl font-bold ${stat.color} mb-1`}>{stat.metric}</p>
